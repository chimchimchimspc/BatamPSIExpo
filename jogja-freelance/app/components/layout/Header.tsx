...
import { Search, Bell, Menu, X, LogOut, MessageCircle } from "lucide-react";
...
export default function Header() {
  ...
  // Poll unread chat count for the header badge (only when logged in).
  useEffect(() => {
    if (!user) {
      setChatUnread(0);
      return;
    ...
    const fetchUnread = () => {
      api.chat
        .conversations()
        .then((rows) => {
          if (active) setChatUnread((rows || []).reduce((sum, c) => sum + (c.unread_count || 0), 0));
        })
        .catch(() => {});
    };
    fetchUnread();
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-6">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            ...
            {chatUnread > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 bg-[#D64545] text-white text-[10px] rounded-full flex items-center justify-center">
